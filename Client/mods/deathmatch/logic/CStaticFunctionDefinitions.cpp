...
bool CStaticFunctionDefinitions::ResetVehicleHandling ( CClientVehicle* pVehicle, bool bUseOriginal )
{
    assert ( pVehicle );

    //if ( bUseOriginal )
    //{
        auto pHandlingEntry = pVehicle->GetOriginalHandlingData ();
        pVehicle->GetHandlingData ()->Assign ( pHandlingEntry );
...
bool CStaticFunctionDefinitions::ResetVehicleHandlingProperty ( CClientVehicle* pVehicle, eHandlingProperty eProperty, bool bUseOriginal )
{
    assert ( pVehicle );

    eVehicleTypes eModel = (eVehicleTypes) pVehicle->GetModel ();
    auto pHandlingEntry = pVehicle->GetHandlingData ();
    ...
    if ( pHandlingEntry )
    {
        ...
        {
            SetEntryHandling ( pHandlingEntry, eProperty, fValue );
        }
        else if ( GetVehicleHandling ( pVehicle, eProperty, uiValue ) )
        {
            SetEntryHandling ( pHandlingEntry, eProperty, uiValue );
        }
        else if ( GetVehicleHandling ( pVehicle, eProperty, ucValue ) )
        {
            SetEntryHandling ( pHandlingEntry, eProperty, ucValue );
        }
        else if ( GetVehicleHandling ( pVehicle, eProperty, strValue ) )
        {
            SetEntryHandling ( pHandlingEntry, eProperty, strValue );
        }
        else if ( GetVehicleHandling ( pVehicle, eProperty, vecValue ) )
        {
            SetEntryHandling ( pHandlingEntry, eProperty, vecValue );
        }
        else
        {
            return false;
    ...
    if ( pEntry )
    {
        switch ( eProperty )
        {
            ...
            case HANDLING_ABS: // bool
                fValue = pEntry->GetABS () ? 1.0f : 0.0f;
                break;
            default:
                return false;
        }
